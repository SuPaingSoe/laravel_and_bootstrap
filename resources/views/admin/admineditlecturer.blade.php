<x-layout>
    ...
    <p align="left" class="text-primary"><strong>Lecturer List</strong><p>
    <div align="right">
    <input type="text" id="myInput" onkeyup="SearchMultiColumns()" placeholder="Search for information..."><br/><br/>
    </div>
    <table id="myTable" class="table table-striped" style="width:100%;" align="center" border="1px">
  <thead>
    <tr>
      ...
      <th valign="middle" scope="col" colspan="2">Action</th>
    </tr>
  </thead>
  ...
  @if($teacher->status =='0')
    <tr>
      <td valign="middle"><a href="/teachers/{{$teacher->id}}">{{$teacher->teacher_name}}</a></td>
      ...
      <td valign="middle"><a href="/admin/{{$teacher->id}}/teacher/edit" class="btn btn-sm btn-success">Edit</a></td>
      <td valign="middle">
      <form action="/admin/{{$teacher->id}}/delete" method="POST">
                @csrf
                    <button type="submit" class="btn btn-primary btn btn-sm">
                      Delete
                    </button>
            </form>
      </td>
