    ...
    for (var i = 0; i < Tables.length; i++) {
        ...
        for (var rowNum = 0; rowNum < THead.rows.length; rowNum++) {
            for (
                ...
                if (
                    THead.rows[rowNum].cells[cellNum].className.match(
                        /\bjs-sort-none\b/
                    )
                ) {
                    continue;
                ...
                window.addEventListener
                    ? THead.rows[rowNum].cells[cellNum].addEventListener(
                          "click",
                          Handler
                      )
                    : window.attachEvent &&
                      THead.rows[rowNum].cells[cellNum].attachEvent(
                          "onclick",
                          Handler
                      );
